...
def process_devide_segment(term, out_json_path, prompt):
    client = OpenAI(
    api_key = os.getenv('OPENAI_API_KEY'),
    )

    message = prompt.replace("{Raw_COT}", term['prediction'])
...
def seg_process(input_jsonl_file, output_jsonl_file):
    
    project_path = os.path.dirname(os.path.abspath(__file__))
    prompt_template_path = os.path.join(project_path, 'templates/template_DeepSeek_R1_QwQ_response_split.txt')
    ...
    with open(input_jsonl_file, 'r', encoding='utf-8') as f:
        ...
        if os.path.exists(output_jsonl_file):
            with open(output_jsonl_file, 'r') as f: # 去除已经处理好的，断点续改
                already = set()
        ...
        # 多进程并行generate
        Parallel(n_jobs=NUM_PROC)(delayed(process_devide_segment)(entry, output_jsonl_file, prompt_template) for entry in tqdm(data_dict))
